=====================
Package documentation
=====================

The notebooks show here contain “technical” documentation for specific features
of the different packages. If you are new to ``ubermag`` you should consider
first looking at :doc:`../getting-started/index` which will give a gentle
introduction into all the packages inside ``ubermag``.

.. toctree::
    :maxdepth: 1
    :caption: Packages
    :hidden:

    discretisedfield
    mag2exp
    micromagneticdata
    micromagneticmodel
    micromagnetictests
    oommfc
    ubermagtable
    ubermagutil
    ubermag

+--------------------+--------------------+--------------------+--------------------+--------------------+--------------------+
|                    | Latest release     | Build              | Coverage           | Binder             | License            |
+====================+====================+====================+====================+====================+====================+
| discretisedfield   | |df_pypi|          | |df_workflow|      | |df_coverage|      | |df_binder|        | |license|          |
|                    +--------------------+--------------------+                    |                    |                    |
|                    | |df_anaconda|      | |df_conda|         |                    |                    |                    |
+--------------------+--------------------+--------------------+--------------------+--------------------+--------------------+
| mag2exp            | |m2_pypi|          | |m2_workflow|      | |m2_coverage|      | |m2_binder|        | |license|          |
|                    +--------------------+--------------------+                    |                    |                    |
|                    | |m2_anaconda|      | |m2_conda|         |                    |                    |                    |
+--------------------+--------------------+--------------------+--------------------+--------------------+--------------------+
| micromagneticdata  | |md_pypi|          | |md_workflow|      | |md_coverage|      | |md_binder|        | |license|          |
|                    +--------------------+--------------------+                    |                    |                    |
|                    | |md_anaconda|      | |md_conda|         |                    |                    |                    |
+--------------------+--------------------+--------------------+--------------------+--------------------+--------------------+
| micromagneticmodel | |mm_pypi|          | |mm_workflow|      | |mm_coverage|      | |mm_binder|        | |license|          |
|                    +--------------------+--------------------+                    |                    |                    |
|                    | |mm_anaconda|      | |mm_conda|         |                    |                    |                    |
+--------------------+--------------------+--------------------+--------------------+--------------------+--------------------+
| micromagnetictests | |mt_pypi|          | |mt_workflow|      | |mt_coverage|      | |mt_binder|        | |license|          |
|                    +--------------------+--------------------+                    |                    |                    |
|                    | |mt_anaconda|      | |mt_conda|         |                    |                    |                    |
+--------------------+--------------------+--------------------+--------------------+--------------------+--------------------+
| oommfc             | |oc_pypi|          | |oc_workflow|      | |oc_coverage|      | |oc_binder|        | |license|          |
|                    +--------------------+--------------------+                    |                    |                    |
|                    | |oc_anaconda|      | |oc_conda|         |                    |                    |                    |
+--------------------+--------------------+--------------------+--------------------+--------------------+--------------------+
| ubermagtable       | |ut_pypi|          | |ut_workflow|      | |ut_coverage|      | |ut_binder|        | |license|          |
|                    +--------------------+--------------------+                    |                    |                    |
|                    | |ut_anaconda|      | |ut_conda|         |                    |                    |                    |
+--------------------+--------------------+--------------------+--------------------+--------------------+--------------------+
| ubermagutil        | |uu_pypi|          | |uu_workflow|      | |uu_coverage|      | |uu_binder|        | |license|          |
|                    +--------------------+--------------------+                    |                    |                    |
|                    | |uu_anaconda|      | |uu_conda|         |                    |                    |                    |
+--------------------+--------------------+--------------------+--------------------+--------------------+--------------------+
| ubermag            | |ub_pypi|          | |ub_workflow|      | |ub_coverage|      | |ub_binder|        | |license|          |
|                    +--------------------+--------------------+                    |                    |                    |
|                    | |ub_anaconda|      | |ub_conda|         |                    |                    |                    |
+--------------------+--------------------+--------------------+--------------------+--------------------+--------------------+

.. discretisedfield
.. |df_pypi| image:: https://badge.fury.io/py/discretisedfield.svg
                     :target: https://badge.fury.io/py/discretisedfield
.. |df_anaconda| image:: https://anaconda.org/conda-forge/discretisedfield/badges/version.svg
                         :target: https://anaconda.org/conda-forge/discretisedfield
.. |df_workflow| image:: https://github.com/ubermag/discretisedfield/workflows/workflow/badge.svg
                         :target: https://github.com/ubermag/discretisedfield/actions?query=workflow%3Aworkflow
.. |df_conda| image:: https://github.com/ubermag/discretisedfield/workflows/conda/badge.svg
                      :target: https://github.com/ubermag/discretisedfield/actions?query=workflow%3Aconda
.. |df_coverage| image:: https://codecov.io/gh/ubermag/discretisedfield/branch/master/graph/badge.svg?token=hcK4fofmrL       
                         :target: https://codecov.io/gh/ubermag/discretisedfield
.. |df_binder| image:: https://mybinder.org/badge_logo.svg
                       :target: https://mybinder.org/v2/gh/ubermag/discretisedfield/latest?urlpath=lab/tree/docs

.. mag2exp
.. |m2_pypi| image:: https://badge.fury.io/py/mag2exp.svg
                     :target: https://badge.fury.io/py/mag2exp
.. |m2_anaconda| image:: https://anaconda.org/conda-forge/mag2exp/badges/version.svg
                         :target: https://anaconda.org/conda-forge/mag2exp
.. |m2_workflow| image:: https://github.com/ubermag/mag2exp/workflows/workflow/badge.svg
                         :target: https://github.com/ubermag/mag2exp/actions?query=workflow%3Aworkflow
.. |m2_conda| image:: https://github.com/ubermag/mag2exp/workflows/conda/badge.svg
                      :target: https://github.com/ubermag/mag2exp/actions?query=workflow%3Aconda
.. |m2_coverage| image:: https://codecov.io/gh/ubermag/mag2exp/branch/master/graph/badge.svg?token=hcK4fofmrL       
                         :target: https://codecov.io/gh/ubermag/mag2exp
.. |m2_binder| image:: https://mybinder.org/badge_logo.svg
                       :target: https://mybinder.org/v2/gh/ubermag/mag2exp/latest?urlpath=lab/tree/docs

.. micromagneticdata
.. |md_pypi| image:: https://badge.fury.io/py/micromagneticdata.svg
                     :target: https://badge.fury.io/py/micromagneticdata
.. |md_anaconda| image:: https://anaconda.org/conda-forge/micromagneticdata/badges/version.svg
                         :target: https://anaconda.org/conda-forge/micromagneticdata
.. |md_workflow| image:: https://github.com/ubermag/micromagneticdata/workflows/workflow/badge.svg
                         :target: https://github.com/ubermag/micromagneticdata/actions?query=workflow%3Aworkflow
.. |md_conda| image:: https://github.com/ubermag/micromagneticdata/workflows/conda/badge.svg
                      :target: https://github.com/ubermag/micromagneticdata/actions?query=workflow%3Aconda
.. |md_coverage| image:: https://codecov.io/gh/ubermag/micromagneticdata/branch/master/graph/badge.svg?token=hcK4fofmrL       
                         :target: https://codecov.io/gh/ubermag/micromagneticdata
.. |md_binder| image:: https://mybinder.org/badge_logo.svg
                       :target: https://mybinder.org/v2/gh/ubermag/micromagneticdata/latest?urlpath=lab/tree/docs

.. micromagneticmodel
.. |mm_pypi| image:: https://badge.fury.io/py/micromagneticmodel.svg
                     :target: https://badge.fury.io/py/micromagneticmodel
.. |mm_anaconda| image:: https://anaconda.org/conda-forge/micromagneticmodel/badges/version.svg
                         :target: https://anaconda.org/conda-forge/micromagneticmodel
.. |mm_workflow| image:: https://github.com/ubermag/micromagneticmodel/workflows/workflow/badge.svg
                         :target: https://github.com/ubermag/micromagneticmodel/actions?query=workflow%3Aworkflow
.. |mm_conda| image:: https://github.com/ubermag/micromagneticmodel/workflows/conda/badge.svg
                      :target: https://github.com/ubermag/micromagneticmodel/actions?query=workflow%3Aconda
.. |mm_coverage| image:: https://codecov.io/gh/ubermag/micromagneticmodel/branch/master/graph/badge.svg?token=hcK4fofmrL       
                         :target: https://codecov.io/gh/ubermag/micromagneticmodel
.. |mm_binder| image:: https://mybinder.org/badge_logo.svg
                       :target: https://mybinder.org/v2/gh/ubermag/micromagneticmodel/latest?urlpath=lab/tree/docs

.. micromagnetictests
.. |mt_pypi| image:: https://badge.fury.io/py/micromagnetictests.svg
                     :target: https://badge.fury.io/py/micromagnetictests
.. |mt_anaconda| image:: https://anaconda.org/conda-forge/micromagnetictests/badges/version.svg
                         :target: https://anaconda.org/conda-forge/micromagnetictests
.. |mt_workflow| image:: https://github.com/ubermag/micromagnetictests/workflows/workflow/badge.svg
                         :target: https://github.com/ubermag/micromagnetictests/actions?query=workflow%3Aworkflow
.. |mt_conda| image:: https://github.com/ubermag/micromagnetictests/workflows/conda/badge.svg
                      :target: https://github.com/ubermag/micromagnetictests/actions?query=workflow%3Aconda
.. |mt_coverage| image:: https://codecov.io/gh/ubermag/micromagnetictests/branch/master/graph/badge.svg?token=hcK4fofmrL       
                         :target: https://codecov.io/gh/ubermag/micromagnetictests
.. |mt_binder| image:: https://mybinder.org/badge_logo.svg
                       :target: https://mybinder.org/v2/gh/ubermag/micromagnetictests/latest?urlpath=lab/tree/docs

.. oommfc
.. |oc_pypi| image:: https://badge.fury.io/py/oommfc.svg
                     :target: https://badge.fury.io/py/oommfc
.. |oc_anaconda| image:: https://anaconda.org/conda-forge/oommfc/badges/version.svg
                         :target: https://anaconda.org/conda-forge/oommfc
.. |oc_workflow| image:: https://github.com/ubermag/oommfc/workflows/workflow/badge.svg
                         :target: https://github.com/ubermag/oommfc/actions?query=workflow%3Aworkflow
.. |oc_conda| image:: https://github.com/ubermag/oommfc/workflows/conda/badge.svg
                      :target: https://github.com/ubermag/oommfc/actions?query=workflow%3Aconda
.. |oc_coverage| image:: https://codecov.io/gh/ubermag/oommfc/branch/master/graph/badge.svg?token=hcK4fofmrL       
                         :target: https://codecov.io/gh/ubermag/oommfc
.. |oc_binder| image:: https://mybinder.org/badge_logo.svg
                       :target: https://mybinder.org/v2/gh/ubermag/oommfc/latest?urlpath=lab/tree/docs

.. ubermagtable
.. |ut_pypi| image:: https://badge.fury.io/py/ubermagtable.svg
                     :target: https://badge.fury.io/py/ubermagtable
.. |ut_anaconda| image:: https://anaconda.org/conda-forge/ubermagtable/badges/version.svg
                         :target: https://anaconda.org/conda-forge/ubermagtable
.. |ut_workflow| image:: https://github.com/ubermag/ubermagtable/workflows/workflow/badge.svg
                         :target: https://github.com/ubermag/ubermagtable/actions?query=workflow%3Aworkflow
.. |ut_conda| image:: https://github.com/ubermag/ubermagtable/workflows/conda/badge.svg
                      :target: https://github.com/ubermag/ubermagtable/actions?query=workflow%3Aconda
.. |ut_coverage| image:: https://codecov.io/gh/ubermag/ubermagtable/branch/master/graph/badge.svg?token=hcK4fofmrL       
                         :target: https://codecov.io/gh/ubermag/ubermagtable
.. |ut_binder| image:: https://mybinder.org/badge_logo.svg
                       :target: https://mybinder.org/v2/gh/ubermag/ubermagtable/latest?urlpath=lab/tree/docs

.. ubermagutil
.. |uu_pypi| image:: https://badge.fury.io/py/ubermagutil.svg
                     :target: https://badge.fury.io/py/ubermagutil
.. |uu_anaconda| image:: https://anaconda.org/conda-forge/ubermagutil/badges/version.svg
                         :target: https://anaconda.org/conda-forge/ubermagutil
.. |uu_workflow| image:: https://github.com/ubermag/ubermagutil/workflows/workflow/badge.svg
                         :target: https://github.com/ubermag/ubermagutil/actions?query=workflow%3Aworkflow
.. |uu_conda| image:: https://github.com/ubermag/ubermagutil/workflows/conda/badge.svg
                      :target: https://github.com/ubermag/ubermagutil/actions?query=workflow%3Aconda
.. |uu_coverage| image:: https://codecov.io/gh/ubermag/ubermagutil/branch/master/graph/badge.svg?token=hcK4fofmrL       
                         :target: https://codecov.io/gh/ubermag/ubermagutil
.. |uu_binder| image:: https://mybinder.org/badge_logo.svg
                       :target: https://mybinder.org/v2/gh/ubermag/ubermagutil/latest?urlpath=lab/tree/docs

.. ubermag
.. |ub_pypi| image:: https://badge.fury.io/py/ubermag.svg
                     :target: https://badge.fury.io/py/ubermag
.. |ub_anaconda| image:: https://anaconda.org/conda-forge/ubermag/badges/version.svg
                         :target: https://anaconda.org/conda-forge/ubermag
.. |ub_workflow| image:: https://github.com/ubermag/ubermag/workflows/workflow/badge.svg
                         :target: https://github.com/ubermag/ubermag/actions?query=workflow%3Aworkflow
.. |ub_conda| image:: https://github.com/ubermag/ubermag/workflows/conda/badge.svg
                      :target: https://github.com/ubermag/ubermag/actions?query=workflow%3Aconda
.. |ub_coverage| image:: https://codecov.io/gh/ubermag/ubermag/branch/master/graph/badge.svg?token=hcK4fofmrL       
                         :target: https://codecov.io/gh/ubermag/ubermag
.. |ub_binder| image:: https://mybinder.org/badge_logo.svg
                       :target: https://mybinder.org/v2/gh/ubermag/ubermag/latest?urlpath=lab/tree/docs

.. |license| image:: https://img.shields.io/badge/License-BSD%203--Clause-blue.svg
                     :target: https://opensource.org/licenses/BSD-3-Clause
